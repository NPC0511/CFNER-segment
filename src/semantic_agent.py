...
class SemanticAgent(object):
    ...
    def _build_llm_plan(self, base_plan, retrieved_evidence, llm_output, raw_output,
        ...
        plan["llm_calibration"] = {"mode": "disabled"}
    ...
    def _apply_reflection_updates(self, risk_edges, domain_name, current_iteration):
        # Reflection remains available for task-end analysis, but a zero weight
        # makes it observational rather than a training-risk override.
        if self.reflection_update_weight <= 0:
            return risk_edges
