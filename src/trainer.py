...
class BaseTrainer(object):
    ...
    def _apply_risk_aware_pseudo_label_thresholds(self, pseudo_labels, confidence, mask_background):
        ...
        fallback_confidence = min(max(
            float(getattr(self.params, "risk_pseudo_label_fallback_confidence", 0.45)), 0.0
        ), 1.0)
    ...
    def batch_loss_rdp(self, labels):
        ...
        Regularizer_soft = self.reg_pesudo_label(self.logits, original_labels)

        valid_token_mask = (original_labels != pad_token_label_id).to(loss_soft_label.dtype)
        valid_token_count = valid_token_mask.sum().clamp_min(1.0)
        loss_soft_label = loss_soft_label.sum() / valid_token_count
