...
class BCEWithLogitsLossWithIgnoreIndexSoftLabel(nn.Module):
    ...
    def forward(self, inputs, old_outputs, old_classes, targets):
        ...
        n_cl = torch.tensor(inputs.shape[-1], device=inputs.device)
        labels_new = torch.where(valid_mask, targets, n_cl)
        ...
        # Ignore padding, special tokens, and non-first subword positions.
        # These positions carry no gold label and must not contribute teacher
        # supervision to the incremental model.
